...
export async function seedClassifieds(prisma: PrismaClient) {
  ...
  const makesCount = await prisma.make.count();
  if (makesCount === 0) {
    console.log("No makes found in database. Please run seedTaxonomy first.");
    return;
  }
  ...
  // Kiểm tra xem có makes nào có models không
  const makesWithModels = makes.filter(make => make.models.length > 0);
  if (makesWithModels.length === 0) {
    console.log("No makes with models found. Please check taxonomy data.");
    return;
  }
  ...
  for (let i = 0; i < 25; i++) {
    const make = faker.helpers.arrayElement(makesWithModels);
    ...
    const vrn = faker.vehicle.vrm();

    const baseSlug = slugify(`${title}-${vrn}`, { lower: true });
  ...
  console.log(`Create ${result.count} classifieds`);
}
