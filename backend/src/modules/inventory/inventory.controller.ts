...
export class InventoryController {
  ...
  async getEVMInventoryByVehicle(
    req: Request,
    res: Response,
    next: NextFunction
  ) {
  ...
  async updateEVMInventory(req: Request, res: Response, next: NextFunction) {
    try {
      const { vehicleId } = req.params;
      const data = await inventoryService.updateEVMInventory(
        vehicleId,
        req.body
      );
  ...
  async getDealerInventoryItem(
    ...
    try {
      ...
      const data = await inventoryService.getDealerInventoryItem(
        dealerId,
        vehicleId
      );
      return ResponseUtil.success(res, data);
  ...
  async updateDealerInventory(req: Request, res: Response, next: NextFunction) {
    try {
      const { dealerId, vehicleId } = req.params;
      const data = await inventoryService.updateDealerInventory(
        dealerId,
        vehicleId,
        req.body
      );
      return ResponseUtil.success(res, data, "Dealer inventory updated");
    } catch (error: any) {
      if (error.message.includes("not found"))
        return ResponseUtil.notFound(res, error.message);
      if (error.message.includes("cannot be negative"))
