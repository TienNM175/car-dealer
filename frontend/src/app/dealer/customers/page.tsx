...
export default function CustomersPage() {
  ...
  const handleDelete = async (customer: Customer) => {
    if (!confirm(`Xác nhận xóa khách hàng ${customer.firstName} ${customer.lastName}?`)) {
      return;
    }
  ...
  return (
    <div className="p-6">
      <CustomerList
        ...
        onViewFeedbacksClick={handleViewFeedbacksClick}
              ...
              <ul className="space-y-3 max-h-96 overflow-y-auto">
                {feedbacks.map((fb) => (
                  <li key={fb.id} className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <p className="text-sm text-gray-700"><strong>Bình luận:</strong> {fb.comment || 'Không có'}</p>
                  </li>
                ))}
              </ul>
            ) : (
              ...
              <ul className="space-y-3 max-h-96 overflow-y-auto">
                {complaints.map((cp) => (
                  <li key={cp.id} className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                    <p className="text-sm text-gray-700 mb-1"><strong>Tiêu đề:</strong> {cp.subject}</p>
