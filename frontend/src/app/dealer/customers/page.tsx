          ...
          <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-semibold text-gray-800">
                Chi tiết khách hàng: {detailCustomer.firstName} {detailCustomer.lastName}
              </h2>
              <button
              ...
              <div className="space-y-3">
                ...
                {detailCustomer.address && (
                  <div>
                    ...
                    <p className="text-gray-900 ml-2">{detailCustomer.address}</p>
                  </div>
                )}
                {detailCustomer.city && (
                  <div>
                ...
                {detailCustomer.identityCard && (
                  <div>
                    ...
                    <p className="text-gray-900 ml-2">{detailCustomer.identityCard}</p>
                  </div>
                )}
              </div>
              <div className="space-y-3">
                <div>
                  ...
                  <span className={`ml-2 px-3 py-1 text-xs font-medium rounded-full ${statusConfig[detailCustomer.status as keyof typeof statusConfig]?.color || 'bg-gray-100 text-gray-700'}`}>
                    {statusConfig[detailCustomer.status as keyof typeof statusConfig]?.label || (detailCustomer.status ? detailCustomer.status.toUpperCase() : 'N/A')}
                  </span>
                </div>
                <div>
                  <strong className="text-gray-700">Ngày tạo:</strong>
                  <p className="text-gray-900 ml-2">
                    {detailCustomer.createdAt ? new Date(detailCustomer.createdAt).toLocaleDateString('vi-VN') || 'N/A' : 'N/A'}
                  </p>
                </div>
                <div>
                  <strong className="text-gray-700">Ngày cập nhật:</strong>
                  <p className="text-gray-900 ml-2">
                    {detailCustomer.updatedAt ? new Date(detailCustomer.updatedAt).toLocaleDateString('vi-VN') || 'N/A' : 'N/A'}
                  </p>
                </div>
              </div>
            </div>
            ...
            {detailCustomer._count && (
              <div className="bg-gray-50 rounded-lg p-4 mb-6">
                <h3 className="text-lg font-semibold text-gray-800 mb-3">Hoạt động liên quan</h3>
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                  <div className="text-center">
                    <p className="text-2xl font-bold text-blue-600">{detailCustomer._count.contracts}</p>
                    <p className="text-sm text-gray-600">Hợp đồng</p>
                  </div>
                  <div className="text-center">
                    <p className="text-2xl font-bold text-green-600">{detailCustomer._count.quotations}</p>
                    <p className="text-sm text-gray-600">Báo giá</p>
                  </div>
                  <div className="text-center">
                    <p className="text-2xl font-bold text-purple-600">{detailCustomer._count.testDrives}</p>
                    <p className="text-sm text-gray-600">Lái thử</p>
                  </div>
                  <div className="text-center">
                    <p className="text-2xl font-bold text-orange-600">{detailCustomer._count.feedbacks}</p>
                    <p className="text-sm text-gray-600">Phản hồi</p>
                  </div>
                  <div className="text-center">
                    <p className="text-2xl font-bold text-red-600">{detailCustomer._count.complaints}</p>
        ...
        <div className="fixed inset-0 backdrop-blur-sm bg-gray-900/30 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md">
            ...
            <p className="text-gray-600 mb-6">
              Bạn có chắc chắn muốn xóa khách hàng <strong>{customerToDelete.firstName} {customerToDelete.lastName}</strong>? Hành động này không thể hoàn tác.
            </p>
