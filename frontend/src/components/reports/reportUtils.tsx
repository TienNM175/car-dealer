...
// --- Hằng số và Types ---
export const COLORS = ["#2563eb", "#10b981", "#f59e0b", "#ef4444", "#8b5cf6", "#14b8a6"];
...
export function BarChartComponent({
    ...
    const wrapTick = (maxChars: number) => {
        const Tick: React.FC<any> = ({ x = 0, y = 0, payload }) => {
            ...
            return (
                <g transform={`translate(${x},${y})`}>
                    <text textAnchor="middle" fontSize={11} fill="#111827" style={{ pointerEvents: "none", lineHeight: "12px" }} y={0}>
                        {finalLines.map((ln, i) => (<tspan key={i} x={0} dy={i === 0 ? 14 : 12}>{ln}</tspan>))}
                        {lines.length > 3 && (<tspan x={0} dy={12}>...</tspan>)}
                    </text>
                </g>
            );
        };
    ...
    return (
        <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} layout={layout}>
                ...
                {layout === "vertical" ? (
                    <YAxis dataKey={xKey} type="category" interval={0} width={200} tick={{ fontSize: 12 }} tickFormatter={(v) => VN_LABELS[v] ?? String(v)} />
                ) : (
                    <XAxis dataKey={xKey} interval={0} height={80} tick={xTick} tickLine={false} minTickGap={8} />
                )}
                {layout === "vertical" ? (
                ...
                <Legend formatter={(v) => `• ${VN_LABELS[v] ?? v}`} />
                {bars.map((b) => (<Bar key={b.key} dataKey={b.key} stackId={stacked ? "a" : undefined} fill={b.color} radius={[4, 4, 0, 0]} />))}
            </BarChart>
        </ResponsiveContainer>
    );
}
    ...
    return (
        <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data}>
                ...
                <Tooltip formatter={(value, name) => [valueFormatter ? valueFormatter(value, name) : value, VN_LABELS[name as string] ?? name]} labelFormatter={(label) => `Nhóm: ${VN_LABELS[label] ?? label}`} />
                <Legend formatter={(v) => `• ${VN_LABELS[v] ?? v}`} />
                {lines.map((l) => (<Line key={l.key} type="monotone" dataKey={l.key} stroke={l.color} strokeWidth={2} dot={false} />))}
            </LineChart>
        </ResponsiveContainer>
    );
}
...
export function LoadingState() {
    return <div className="h-[200px] flex items-center justify-center text-black">Đang tải dữ liệu...</div>;
}

export function ErrorState({ message }: { message: string }) {
    return <div className="h-[200px] flex items-center justify-center text-red-600">{message}</div>;
}
...
export const StatCard = ({ label, value, color }: { label: string; value: number | string; color?: string }) => (
    <div className={`shadow rounded-xl p-6 text-center ${color ?? "bg-white"}`}>
        ...
        <p className="text-xl font-bold text-black">{toNum(value).toLocaleString()}</p>
    </div>
);

export const SectionCard = ({ title, children, color }: { title: string; children: React.ReactNode; color?: string }) => (
    <div>
        <div className={`bg-gradient-to-r ${color} text-white px-4 py-2 rounded-t-lg font-semibold`}>{title}</div>
