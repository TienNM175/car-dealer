...
const StatCard = ({
  ...
  return (
    <div
      className={`bg-white rounded-lg shadow p-4 border-l-4 ${colorClasses[color]}`}
    >
      ...
      <div className="flex justify-between items-start mb-3">
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-600 mb-1">
            {debt.vehicle?.manufacturer?.name} {debt.vehicle?.model}{" "}
            {debt.vehicle?.variant}
          </p>
          <p className="text-sm text-gray-500">
            HĐ: {debt.contract?.contractCode} •{" "}
            {formatDate(debt.contract?.createdAt)}
          </p>
        </div>
...
export default function ReportDealer({
  ...
  const renderSales = (d: ReportPayload) => {
    ...
    // Tiền hủy cọc (chi tiết cho đại lý)
    const cancelledDeposits = d.cancelledDeposits || {
      totalAmount: 0,
      count: 0,
    };
    ...
    return (
      <div className="grid grid-cols-1 gap-8">
        {/* StatCard cho tiền hủy cọc */}
        {cancelledDeposits.totalAmount > 0 && (
          <div className="bg-white rounded-xl shadow p-6 border-l-4 border-orange-500">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-sm text-gray-600">Tổng tiền hủy cọc</p>
                <p className="text-2xl font-bold text-orange-600">
                  {new Intl.NumberFormat("vi-VN", {
                    style: "currency",
                    currency: "VND",
                  }).format(Number(cancelledDeposits.totalAmount || 0))}
                </p>
              </div>
              <div>
                <p className="text-sm text-gray-600">Số HĐ đặt cọc đã hủy</p>
                <p className="text-2xl font-bold text-gray-900">
                  {cancelledDeposits.count || 0}
                </p>
              </div>
            </div>
            <p className="text-xs text-gray-500 mt-2">
              * Tiền cọc không hoàn lại khi khách hủy HĐ đặt cọc
            </p>
          </div>
        )}
