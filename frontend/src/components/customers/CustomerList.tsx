...
export default function CustomerList({
  ...
  // ✅ Calculate stats from current page OR use stats from backend
  const statusCounts = React.useMemo(() => {
    // If stats provided from backend, use those
    if (stats) {
      return {
        total: stats.total,
    ...
    // Otherwise calculate from current page (temporary, not accurate)
    const counts: Record<string, number> = {
      total: pagination.total, // ✅ Use pagination.total for accurate count
      PURCHASED: 0,
      INTERESTED: 0,
      COLD: 0,
    };
    ...
    return counts;
  }, [customers, pagination.total, stats]);

  // ✅ Generate pagination buttons with proper logic
    ...
    if (startPage > 1) {
      buttons.push(
        <button
          ...
          className="px-3 py-1 border border-gray-300 rounded hover:bg-gray-50 text-black"
        >
          1
        </button>
      );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-md p-6">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">
                            CCCD: {customer.identityCard || 'N/A'}
                          </p>
                        </div>
                      </div>
                    </td>
        ...
        {/* ✅ FIXED Pagination */}
        {!loading && customers.length > 0 && pagination.totalPages > 0 && (
