...
export default function DealerOrderForm({
  ...
  useEffect(() => {
    const fetchVehicles = async () => {
      try {
        const res = await vehicleApi.getAllVehicles({ status: 'ACTIVE' }, { limit: 100 });
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {selectedVehicle && (
            <div className="bg-green-50 p-4 rounded-lg text-green-800">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                ...
                <div>
                  <span className="text-green-700">Model:</span> {selectedVehicle.model}
                </div>
                <div>
                  <span className="text-green-700">Phiên bản:</span> {selectedVehicle.variant}
                </div>
                <div>
                  <span className="text-green-700">Năm:</span> {selectedVehicle.year}
                </div>
                <div>
          ...
          {selectedVehicle && (
            <div className="bg-yellow-50 p-4 rounded-lg text-yellow-800">
              <h4 className="font-medium text-yellow-900 mb-2">Tính toán giá</h4>
