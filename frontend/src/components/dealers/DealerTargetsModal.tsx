...
import { X, Target, TrendingUp, Calendar, Plus, Edit, Save, DollarSign } from 'lucide-react';
...
function TargetEditModal({ 
  ...
  target?: { year: number; month: number; targetAmount: number; id?: string } | null;
  dealerName: string;
  existingTargets: { year: number; month: number; id?: string }[];
}) {
  const [year, setYear] = useState(target?.year || new Date().getFullYear());
  const [month, setMonth] = useState(target?.month || new Date().getMonth() + 1);
  const [targetAmount, setTargetAmount] = useState(target?.targetAmount?.toString() || '');
  ...
  const handleSave = () => {
    ...
    if (!target?.id) {
      const isDuplicate = existingTargets.some(t => 
        t.year === year && t.month === month && t.id !== target?.id
      );
      if (isDuplicate) {
        setError('Đã có chỉ tiêu cho tháng này. Vui lòng chọn tháng khác hoặc chỉnh sửa chỉ tiêu hiện có.');
        return;
      }
    }
...
export default function DealerTargetsModal({ dealer, onClose, onSuccess }: DealerTargetsModalProps) {
  ...
  const [error, setError] = useState('');
  const [editModal, setEditModal] = useState<{isOpen: boolean; target: DealerTarget | null}>({isOpen: false, target: null});
  ...
  const handleSaveTarget = async (year: number, month: number, targetAmount: number) => {
    try {
      const response = await dealerApi.setDealerTarget(dealer.id, year, month, targetAmount);
      if (response.success) {
        onSuccess?.(editModal.target ? 'Đã cập nhật chỉ tiêu thành công' : 'Đã thêm chỉ tiêu thành công');
        loadTargets();
            ...
            <div className="space-y-4">
              {targets.length === 0 ? (
                ...
                getSortedTargets().map((target: DealerTarget, index: number) => {
                  ...
                  return (
                    <div key={target.id || index} className="bg-gray-50 rounded-xl p-4 hover:bg-gray-100 transition-colors">
